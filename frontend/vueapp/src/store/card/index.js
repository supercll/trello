...
export default {
  ...
  getters: {
    ...
    getDoneCards: ({ cards }) => boardListId =>
      cards.filter(card => {
        return card.boardListId == boardListId && card.status;
      }),
    getUndoneCards: ({ cards }) => boardListId =>
      cards.filter(card => card.boardListId == boardListId && !card.status)
  },
  mutations: {
